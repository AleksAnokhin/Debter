...
class Pretrial_entity extends Model_debter implements Debter {
	...
	public function getInfo() {
		$tables = ['collection_stage','main_parameters','pledged_property','pretrial_stage_entity','type_of_debter'];
		$cond = ['collection_stage'=>'debters.id = collection_stage.debters_id','main_parameters'=>'collection_stage.debters_id=main_parameters.debters_id',
				'pledged_property'=>'main_parameters.debters_id=pledged_property.debters_id',
				 'pretrial_stage_entity'=>'pledged_property.debters_id=pretrial_stage_entity.debters_id',
				'type_of_debter'=>'pretrial_stage_entity.debters_id=type_of_debter.debters_id'];
		$id = $this->id;
		$maintable = 'debters';
		...
		$tables = ['collection_stage','main_parameters','pretrial_stage_entity','type_of_debter'];
		$cond = ['collection_stage'=>'debters.id = collection_stage.debters_id','main_parameters'=>'collection_stage.debters_id=main_parameters.debters_id',
				 'pretrial_stage_entity'=>'main_parameters.debters_id=pretrial_stage_entity.debters_id',
				'type_of_debter'=>'pretrial_stage_entity.debters_id=type_of_debter.debters_id'];
		$id = $this->id;
		$maintable = 'debters';
		$select = Model_result::select($maintable,$cond,$tables,$id);
	...
	public function factorsAnalysis($select,$total_summ) {
		...
		if($select['analysis'] == 'no_finance' or $select['analysis'] == 'damages') {
			$minus++;
		} else if($select['analysis'] == 'has_income') {
			$plus++;
		}
		if($select['restructuring_readiness'] == 'yes') {
			$plus++;
		} else if($select['restructuring_readiness'] == 'no') {
			$minus ++;
		}
		return $plus - $minus;
	}
