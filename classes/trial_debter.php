...
class Trial_debter extends Model_debter
    implements Debter
{
    ...
    {
        $tables = ['collection_stage', 'main_parameters', 'trial_pledged_property', 'trial_stage', 'type_of_debter'];
        $cond = ['collection_stage' => 'debters.id = collection_stage.debters_id', 'main_parameters' => 'collection_stage.debters_id=main_parameters.debters_id',
            ...
            'trial_stage' => 'trial_pledged_property.debters_id=trial_stage.debters_id',
            'type_of_debter' => 'trial_stage.debters_id=type_of_debter.debters_id'
        ];
        $id = $this->id;
        $maintable = 'debters';
        ...
        if ($select) {
            ...
            $tables = ['collection_stage', 'main_parameters', 'trial_stage', 'type_of_debter'];
            $cond = ['collection_stage' => 'debters.id = collection_stage.debters_id',
                ...
                'trial_stage' => 'main_parameters.debters_id=trial_stage.debters_id',
                'type_of_debter' => 'trial_stage.debters_id=type_of_debter.debters_id'
            ];
            $id = $this->id;
            $maintable = 'debters';
    ...
    public function factorsAnalysis($select, $total_summ)
    {
        ...
        if ($select['prospects'] == 'low') {
            $minus++;
        } else if ($select['prospects'] == 'high') {
            $plus++;
        }
        if ($select['terms'] == '12') {
            $minus++;
        } else if ($select['terms'] == '3') {
            $plus++;
        }
        return $plus - $minus;
    }
